...
> Questa query non è stata mostrata in aula, ed è riportata qui solo come ulteriore esempio
...
mentre realizzarla senza EXISTS richiederebbe una struttura molto più complessa (ad esempio una differenza insiemistica).

## Sotto-query nella clausola FROM

> Le query di questa sezione non sono state mostrate in aula, e sono riportate qui solo come ulteriori esempi
...
> Le Common Table Expressions non sono state trattate in questa edizione del corso, e quindi le query di questa sezione non sono state mostrate in aula, ma sono  riportate qui come ulteriori esempi di approfondimento
...
Ma se la partita è terminata senza punti segnati dalla squadra in oggetto? In tal caso non esistono record di *segna* che escono dal JOIN-WHERE, quindi la somma nella SELECT restituirà *null*, mentre per noi in questo caso la funzione dovrebbe restituire uno zero. Ci sono molti modi per gestire questo caso particolare: uno molto "comodo" è usare la funzione di MySQL *coalesce*, che restituisce il *primo dei suoi argomenti con valore non nullo*:
...
idpartita integer unsigned, idsquadra integer unsigned)
  RETURNS integer DETERMINISTIC
BEGIN
 RETURN coalesce((SELECT sum(abs(e.punti))
  FROM segna e
   JOIN formazione f ON (e.ID_giocatore = f.ID_giocatore)
   JOIN partita p ON (p.ID = e.ID_partita)
   JOIN campionato c ON (c.ID = p.ID_campionato)
  WHERE (f.anno = c.anno) AND (p.ID=idpartita)
 AND (f.ID_squadra=idsquadra)),0);
END$
```
...
> Questa procedura non è stata mostrata in aula, ed è riportata qui solo come ulteriore esempio
...
FOR EACH ROW BEGIN
 ...
 UPDATE partita
  SET punti_squadra_1=greatest(0,cast(punti_squadra_1 AS SIGNED)+IF(s=ID_squadra_1,abs(NEW.punti),0)),
      punti_squadra_2=greatest(0,cast(punti_squadra_2 AS SIGNED)+IF(s=ID_squadra_2,abs(NEW.punti),0))

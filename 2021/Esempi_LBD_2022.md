...
<title>Esempi di query, procedure, funzioni e trigger relativi al database "Campionati"</title>
</head><body>

Corso di Laboratorio di Basi di Dati  
Esempi di query, procedure, funzioni e trigger  
relativi al database "Campionati" {.titolo}

Autore: G. Della Penna  
Versione 20220920  {.sottotitolo}
...
```sql
SELECT * FROM partita, partita ERRATO!
```
...
in questo caso avremo un record con nome, cognome e squadra pe ogni punto segnato. *Nota: se volessimo considerare gli autogol (ad esempio impostando al valore "AUT" il campo tipo dei record della tabella segna), potremmo filtrarli da questa classifica (gli autogol non contano nelle classifiche dei marcatori!) inserendo il filtro e.tipo\<\>'AUT'.*
...
```sql
SELECT s.nome AS squadra, count(*) AS punti
 FROM segna e
  JOIN giocatore g ON (e.ID_giocatore = g.ID)
  JOIN formazione f ON (f.ID_giocatore = g.ID)
  JOIN squadra s ON (f.ID_squadra = s.ID)
...
Il trucco qui consiste nel sostituire l'espressione del JOIN con la squadra, che a tutti gli effetti ci restituisce, per ogni punto segnato, il nome e l'ID della squadra da usare per il calcolo del punteggio, con un'espressione condizionale complessa, che ovviamente renderà più "pesante" il JOIN, ma realizza nella maniera più semplice quanto desideriamo. In particolare, inseriamo nel JOIN la squadra citata dalla formazione (come nel caso base) se il tipo di punto non è "AUT", altrimenti confrontiamo la squadra a cui appartiene il giocatore (f.ID_squadra) con quelle in partita (p.ID_squadra_1 e p.ID_squadra_2) e inseriamo l'altra squadra. In questo modo, nel JOIN realizzato, indipendentemente da quanto riportato dalla formazione, la squadra inserita sarà quella a vantaggio della quale è stato segnato il punto, e il resto della query (che raggruppa in base alla squadra) potrà rimanere identico.
...
In questo caso, avremmo potuto rendere esplicita la condizione "almeno un punto" usando la clausola HAVING. Tale formulazione sarebbe diventata in ogni caso necessaria se avessimo voluto estrarre i giocatori che hanno segnato almeno 2 punti, ad esempio:
...
### "I giocatori non hanno segnato nel 2020"
...
mentre realizzarla senza EXISTS richiederebbe una struttura più complessa.
...
e unendole in un'unica lista. Come? Usando la UNION di SQL:
...
L'astuzia qui è nell'espressione *(anno IS NULL OR f.anno=anno)* , che fa valutare il vincolo *f.anno=anno* solo se anno non è nullo.
...
Vogliamo realizzare una funzione che ritorna la chiave di un arbitro dati il suo nome, cognome e codice fiscale oppure, se l'arbitro non esiste, lo crea. Nel nostro caso la chiave è proprio il codice fiscale, ma anche questo dettaglio può essere indifferente all'utente, che si dovrà limitare a chiamare la procedura che segue:
...
La funzione prima prova a cercare l'arbitro tramite il suo codice fiscale, e se non lo trova lo inserisce, quindi ritorna il codice fiscale stesso, che nel nostro caso è proprio la chiave dell'arbitro.
...
```sql
CALL aggiungi_partita(2, 3, "2020-12-13 12:12", 1, 1, "Pinco", "Pallino", "abcdefghi", @idp)
```
...
```sql
SELECT f.ID_squadra, count(*) AS punti
 FROM segna e
  JOIN formazione f ON (e.ID_giocatore = f.ID_giocatore)
  JOIN partita p ON (p.ID = e.ID_partita)
  JOIN campionato c ON (c.ID = p.ID_campionato)
...
Di questa possiamo anche scrivere una variante che prende in considerazione gli autogol, ad esempio
...
AS squadra_effettiva, count(*) AS punti
 FROM segna e
  ...
  JOIN partita p ON (p.ID = e.ID_partita)
  JOIN campionato c ON (c.ID = p.ID_campionato)
 WHERE p.ID=1 AND f.anno=c.anno
 GROUP BY squadra_effettiva;
```

In questo caso raggruppiamo su una colonna calcolata che, in base al tipo di punto, contiene l'ID della squadra a cui questo va effettivamente assegnato (la squadra per cui gioca chi ha segnato il punto, tranne nel caso in cui questo sia di tipo "AUT", nel qual caso viene selezionata l'altra squadra nella partita).
...
Quello che facciamo è semplicemente calcolare in che squadra gioca chi ha segnato il punto e poi aggiornare il punteggio della partita di conseguenza (usando il trucco del singolo UPDATE sui due punteggi con un valore calcolato tramite IF, già usato in altre query).

Anche in questo caso, volendo gestire anche gli autogol, la formulazione della query interna al trigger diventerebbe più complessa. Ad esempio, potremmo riscriverla come segue:
...
Tuttavia, negli esempi che seguono, per semplicità useremo la formulazione della query senza autogol.
...
* Dopo un INSERT si aumenta di uno il punteggio della squadra che ha segnato

* Dopo un DELETE si toglie uno al punteggio della squadra corrispondente al punto che viene cancellato

* Dopo un UPDATE si toglie uno al punteggio della squadra corrispondente al "vecchio" record e si aumenta di uno il punteggio della squadra corrispondente al "nuovo" record.

Ovviamente anche qui ci conviene modularizzare il codice per evitare ridondanze. Estraiamo quindi il codice che calcola la squadra di appartenenza del giocatore e somma un numero (qui parametrico) al corrispondente punteggio nella partita:
...
CREATE TRIGGER aggiorna_punti_i AFTER INSERT ON segna
FOR EACH ROW BEGIN
 CALL step_punti(NEW.ID_partita,NEW.ID_giocatore,1);
END$
```
...
FOR EACH ROW BEGIN
 CALL step_punti(OLD.ID_partita,OLD.ID_giocatore,-1);
END$
```
...
Da notare come lo step è -1 nel caso in cui (DELETE e prima parte della UPDATE) si debba togliere un punto invece di aggiungerlo.
...
```sql
INSERT INTO segna(ID_giocatore, ID_partita, minuto) VALUES (1, 1, 1);
```
...
Per chiudere la transazione e confermarne le modifiche apportate al database, avremmo invece ovviamente dovuto inviare il comando COMMIT.
